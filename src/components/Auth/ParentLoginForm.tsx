...
export default function ParentLoginForm({ onShowRegister, onShowForgotPassword }: ParentLoginFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!nisnStudentInfo || nisnError) {
      setError(nisnError || 'NISN tidak valid atau tidak ditemukan.');
      return;
    }

    try {
      const success = await login(nisn, formData.password, 'parent');
      ...
      console.error("ParentLoginForm handleSubmit error:", err);
